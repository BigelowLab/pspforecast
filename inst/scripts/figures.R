## Script for generating figures with pspforecast predictions

figure_dir <- file.path("~/Documents/figures/pspforecast")
...
cm_facet <- ggplot2::ggplot(data = cm, ggplot2::aes(x = .data$predicted, y = .data$actual)) +
...
ggsave(file.path(figure_dir, "prediction_cm_facet.jpg"), cm_facet)


scatter_facet <- ggplot2::ggplot(data = pred_w_results, ggplot2::aes(x=.data$p_3, y=.data$toxicity)) +
...
ggsave(file.path(figure_dir, "prediction_scatter_facet.jpg"), scatter_facet)
