...
ggplot2::ggplot(data = cm, ggplot2::aes(x = .data$actual, y = .data$predicted)) +
  ...
  ggplot2::scale_fill_gradient(low = "white", 
                               high = "blue") +
  ggplot2::labs(y = "Predicted Classifications", 
                x = "Actual Classifications") +
...
ggplot2::ggplot(data = pred_w_results, ggplot2::aes(x=.data$toxicity, y=.data$p_3)) +
  ggplot2::geom_point(alpha=0.7, size=3) +
  ggplot2::facet_grid(cols=vars(.data$year)) +
  ggplot2::labs(y = "Predicted Probability of Closure-level Toxicity (%)",
                x = "Measured Toxicity (μg STX 100g-1 shellfish)") +
