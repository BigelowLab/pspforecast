...
results22 <- add_forecast_results(predictions22, psp) |>
    ...
    rename(prob_0 = p_0,
           ...
           prob_2 = p_2,
           prob_3 = p_3) |>
    select(-ensemble_n, -p_3_max, -p_3_min)
...
dummy_all <- readr::read_csv(system.file(file.path("forecastdb/psp_forecast_all_predictions_2022.csv.gz"), package="pspforecast")) |>
  filter(between(date, as.Date("2022-05-22"), as.Date("2022-05-25")))
    ...
    tabsetPanel(
        tabPanel("Seasonal",
                 ...
                 mainPanel(verticalLayout(
                     ...
                     plotOutput("seasonalConfusion",
                                width="80%"),
                     h3("Closure risk vs measured toxicity - with uncertainty as SD"),
                     plotOutput("seasonalScatterUncertainty"))
                 ))
        ),
        tabPanel("Weekly",
        ...
        tabPanel("Current Forecast",
                 ...
                 mainPanel(
                     ...
                     h3("Closure-level FOrecast Uncertainty"),
                     plotOutput("current_forecast_uncertainty",
                                width="80%"))
                 )
    )
)
...
server <- function(input, output) {
    ...
    output$seasonalScatterUncertainty <- renderPlot({
      results <- predictions |>
        filter(season == input$season)
      
      ggplot(results, aes(x=prob_3, y=toxicity, size=p3_sd)) +
        geom_point()
    })
    ...
    output$current_forecast_uncertainty <- renderPlot({
      dummy_all |>
        ggplot(aes(x=prob_3, y=reorder(location, prob_3, FUN=max))) +
        geom_boxplot()
    })
