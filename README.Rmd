...
## Metrics:
 ...
 + fn - The model predicted class 0,1,2 and the following week's measurement was in class 0,1,2
 + precision - TP/TP+FP
...
tn <- pred_w_results |>
  filter(predicted_class == class & class != 3) |>
  nrow()
...
fn = pred_w_results %>% 
  dplyr::filter(.data$predicted_class != 3 & .data$class != .data$predicted_class) %>% 
  nrow()
